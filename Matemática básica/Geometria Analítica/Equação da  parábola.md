...
\sqrt{x^2 + (y-p)^2} = y + p
$$
Elevando ambos os termos ao quadrado e simplificando, teremos:
$$
x^2 = 4py \ \ \ \ \text{ou} \ \ \ \ y = \dfrac{1}{4p}x^2
$$
Podemos analisar parábolas que não estejam em sua forma canônica, ou seja, que estejam com seu vértice em outro ponto que não seja a origem do sistema, para isso, podemos utilizar o [[Método de completar quadrados]] e realizar uma [[Translação do sistema]], obtendo uma equação para a parábola deslocada da origem mas com eixo paralelo ao eixo y:
$$
y-y_0 = (x-x_0)^2
$$
Algumas parábolas, seguindo as mesmas equações descritas podem ter seu eixo paralelo ao eixo $x$, nesse caso, só trocamos as variáveis:
$$
x-x_0 = (y-y_0)^2
$$