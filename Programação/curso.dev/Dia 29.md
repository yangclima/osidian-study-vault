...
```
O último problema é: O Jest possui um test timeout parão de 5 segundos, ou seja, se algo demorar mais que 5 segundos para executar ele simplesmente parará a suite e lançará um erro, para resolver isso definimos um tempo maior para a opção `testTimeout` no arquivo `jest.config.js`: 
```node
