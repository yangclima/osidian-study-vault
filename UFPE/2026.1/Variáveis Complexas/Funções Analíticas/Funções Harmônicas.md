...
$$\nabla^2\phi = \dfrac{\partial^2\phi}{\partial x^2} + \dfrac{\partial^2\phi}{\partial x^2} = 0$$
...
> Seja $f(z) = u(x,y) + iv(x,y)$ uma [[Funções Complexas|função complexa]] [[Diferenciabilidade e Analiticidade|analítica]] em um domínio $D$, então ambas as funções $u$ e $v$ são harmônicas em $D$
